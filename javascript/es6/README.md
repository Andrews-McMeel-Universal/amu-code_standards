...
| `amuGame.event`                    | object                   | Emits requested event data to the page per `onEvent` options above. If the page requests `"all"`, the game should emit data for all events, otherwise it should emit only the requested event data that matches an available key, such as `"start"`. When emitting an event, also include `amuGame.state`. |
