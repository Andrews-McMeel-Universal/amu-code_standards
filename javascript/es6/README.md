...
| `amuGame.state.resetLevel`         | boolean (required)          | `true` if the user reset the game level state. This action sets everything except for `resetLevel` back to initial values, including the game timer, whether the user has changed completion modes or used hints, and any other progress/settings. This should persist in the game state once set to `true`, even though all other values are reset. |
